...
class HRController {
  ...
  static getMarketResources = (req, res) => {
    try {
      ...
      const resources = statements.db.prepare(query).all(...params);
  ...
  static hireResource = (req, res) => {
    try {
      ...
      // Get the resource from market
      const resource = statements.db.prepare('SELECT * FROM market_resources WHERE id = ? AND is_hired = false').get(resourceId);
