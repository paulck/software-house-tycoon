...
echo "🔧 Final fix for database.js..."

# Backup del database esistente se esiste
if [ -f "database/game.db" ]; then
    mv database/game.db database/game.db.backup
    echo "📦 Database backed up"
fi

# Fix definitivo del database.js
...
# Aggiorna anche il server.js per essere sicuri
...
console.log('🚀 Starting Software House Tycoon Backend...');
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`🗄️  Database: ${process.env.DB_PATH || './data/game.db'}`);
});

module.exports = app;
EOF

echo "✅ Database and server fixed!"
echo ""
